...
METRICS_TICKER_TYPES = {"Stock", "ETF", "Index"}
PRICE_ONLY_TYPES = {"FXPair", "Futures"}
...
# -----------------------------------------------
# 2) DOWNLOAD DAILY OHLCV FROM FMP
# -----------------------------------------------
...
def fetch_daily(symbol, date_from, date_to):
...
# -----------------------------------------------
# 2b) SEPARATE price-only tickers from metrics tickers
# -----------------------------------------------
...
MAX_VALID_VOLUME = 100_000_000_000


def ols_slope_window(arr):
...
def max_drawdown_duration_only(arr):
    a = np.asarray(arr, dtype=float)
    if not np.isfinite(a).any():
        return np.nan
    max_price = np.nan
...
def per_ticker_metrics_all_rows(g):
    g = g.copy()
...
for tkr, g in combined.groupby("ticker", sort=False):
    if not (pd.to_datetime(g["dt"]).dt.date == T_DATE).any():
        continue
    all_rows_out = per_ticker_metrics_all_rows(g)
    day_row = all_rows_out[pd.to_datetime(all_rows_out["date"]).dt.date == T_DATE]
...
if not data_price_only.empty:
    ...
    if "volume" in df_po.columns:
        ...
        df_po.loc[df_po["volume"] >= MAX_VALID_VOLUME, "volume"] = np.nan

    cols_sql = ", ".join(["ticker_id", "dt"] + price_cols_available)
    upd_sql = ", ".join([f"{c} = EXCLUDED.{c}" for c in price_cols_available])
    rows = [_safe_row(r) for r in df_po.itertuples(index=False, name=None)]
...
if not daily_metrics.empty:
    ...
    if present_price:
        ...
        if "volume" in df_price.columns:
            ...
            df_price.loc[df_price["volume"] >= MAX_VALID_VOLUME, "volume"] = np.nan

        cols_sql = ", ".join(["ticker_id", "dt"] + present_price)
        upd_sql = ", ".join([f"{c} = EXCLUDED.{c}" for c in present_price])
        rows = [_safe_row(r) for r in df_price.itertuples(index=False, name=None)]
    ...
    if metric_cols:
        ...
        mrows = [_safe_row(r) for r in long_metrics[["ticker_id", "dt", "metric_id", "value"]].itertuples(index=False, name=None)]
...
# -----------------------------------------------
# 8) Refresh snapshot_metric_latest
# -----------------------------------------------
with pg_conn.cursor() as cur:
    cur.execute(
        f"""
